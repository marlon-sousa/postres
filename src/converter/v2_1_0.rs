...
use crate::{error::PostresError, http_request};

use super::make_base_name;

pub(crate) fn convert_to_http(spec: &v2_1_0::Spec) -> http_request::HttpRequests {
    ...
    /*
        ...
        This is going to be a long read, but I really hopeit will worth it.
        In a very summarized way, iterators let you iterate (go through) through all items of a collection
        ...
        Iterators are, again, kind of simple: you call next() on them and this method either returns Some(item) with the next item or None, in which case there are no more items.
        In fact, for loops are just a sugar syntax to obtaining an iterator on the wanted collection and calling next on them until None is returned.
        ...
        .map(| i | i.to_string())
        // now, we get an item and return an string
        .collect();
...
fn handle_errors(res: &Vec<Result<http_request::HttpRequest, PostresError>>) {
...
fn to_http_headers(postman_headers: Option<&Vec<v2_1_0::Header>>) -> Vec<http_request::Header> {
    let headers = vec![];
    let postman_headers = postman_headers.unwrap_or(&headers);
    postman_headers
        .iter()
        .map(|h| http_request::Header {
            name: h.key.clone(),
            value: h.value.clone(),
        })
        .collect()
